...
// بندهای ۸۵/۸۶ (لِین ۷): جمعِ دقایقِ جلساتِ کارِ پومودورو در N روزِ اخیر —
// pomodoroStatsFor (قلمروِ لِین ۴، الان merge‌شده و پایدار) خودش «امروز»
// و «کل» را می‌دهد ولی «هفته‌ی اخیر به دقیقه» را نه؛ به‌جای دست‌بردن در
// آن تابعِ مشترک، همین یک تابعِ کوچکِ مستقل نوشته شد که دقیقاً همان
// قراردادِ فیلترِ session (`type==="work"`, `completedAt`) را تکرار
// می‌کند.
function weeklyFocusMinutes(sessions, days) {
  const keys = new Set(lastNDays(days || 7).map((d) => d.key));
  return (sessions || []).reduce((sum, s) => {
    if (s.type !== "work" || !s.completedAt) return sum;
    const key = s.completedAt.slice(0, 10);
    return keys.has(key) ? sum + (s.durationMin || 0) : sum;
  }, 0);
}
// بندهای ۸۵/۸۶: پیشرفتِ هدفِ روزانه/هفتگیِ تمرکز نسبت به جلساتِ واقعیِ
// پومودورو. فقط وقتی حداقل یکی از دو هدف تنظیم شده (>۰، طبقِ قراردادِ
// DEFAULT_SCHEDULING.focusGoals) چیزی رندر می‌شود. کاملاً read-only —
// هیچ‌جا state نمی‌نویسد.
function FocusGoalProgress({ pomodoro, scheduling }) {
  const goals = scheduling && scheduling.focusGoals;
  if (!goals || !goals.dailyMinutes && !goals.weeklyMinutes) return null;
  const stats = pomodoroStatsFor(pomodoro.sessions || [], 7);
  const weekly = weeklyFocusMinutes(pomodoro.sessions || [], 7);
  const rows = [];
  if (goals.dailyMinutes > 0) rows.push({ label: "\u0647\u062F\u0641 \u0627\u0645\u0631\u0648\u0632", actual: stats.todayMinutes, goal: goals.dailyMinutes });
  if (goals.weeklyMinutes > 0) rows.push({ label: "\u0647\u062F\u0641 \u0627\u06CC\u0646 \u0647\u0641\u062A\u0647", actual: weekly, goal: goals.weeklyMinutes });
  return /* @__PURE__ */ React.createElement(GlassCard, { className: "p-3 space-y-2" }, rows.map((r, i) => {
    const pct = Math.min(100, Math.round(r.actual / r.goal * 100));
    const reached = r.actual >= r.goal;
    return /* @__PURE__ */ React.createElement("div", { key: i }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between text-[11px] text-slate-400 mb-1" }, /* @__PURE__ */ React.createElement("span", null, r.label), /* @__PURE__ */ React.createElement("span", { style: { color: reached ? "#34D399" : "var(--text-normal)" } }, `${formatDurationFa(r.actual)} \u0627\u0632 ${formatDurationFa(r.goal)}`)), /* @__PURE__ */ React.createElement("div", { className: "h-1.5 rounded-full bg-white/[0.06] overflow-hidden" }, /* @__PURE__ */ React.createElement("div", { className: "h-full rounded-full", style: { width: `${pct}%`, background: reached ? "#34D399" : "#C026D3" } })));
  }));
}
function PomodoroHub({ pomodoro, setPomodoro, tasks, onAddProgress, onToggle, lang, notifSettings, onFocusChange, scheduling }) {
  const [sub, setSub] = useState("timer");
  const term = pomodoro.settings.termFa || "\u067E\u0648\u0645\u0648\u062F\u0648\u0631\u0648";
  return /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement(FocusGoalProgress, { pomodoro, scheduling }), /* @__PURE__ */ React.createElement(SubTabs, { value: sub, onChange: setSub, options: [["timer", `\u062A\u0627\u06CC\u0645\u0631 ${term}`, "clock"], ["report", `\u06AF\u0632\u0627\u0631\u0634 ${term}`, "trending-up"]] }), sub === "timer" && /* @__PURE__ */ React.createElement(PomodoroTimerView, { pomodoro, setPomodoro, tasks, onAddProgress, onToggle, notifSettings, onFocusChange }), sub === "report" && /* @__PURE__ */ React.createElement(PomodoroReportView, { pomodoro, tasks }));
}
