...
function buildDailyMarkdownReport({ tasks, projects, pomodoro }) {
  ...
  const todaysPlan = (tasks || []).filter((tk) => isTaskDueOn(tk, now));
  const doneToday = todaysPlan.filter((tk) => tk.status === "done" && tk.completedDate === today);
  const pendingToday = todaysPlan.filter((tk) => tk.status !== "done");
  const sessions = (pomodoro && pomodoro.sessions || []).filter((s) => s.startedAt && dateKeyOf(new Date(s.startedAt)) === today);
  ...
  (projects || []).forEach((topic) => {
    (topic.subsections || []).forEach((sec) => {
      const task = (tasks || []).find((tk) => tk.id === sec.linkedTaskId);
      if (!task) return;
      const loggedToday = (task.progressLog || []).filter((e) => e.date === today);
      if (loggedToday.length === 0 && !sec.quotaPerPeriod) return;
      const todaySum = loggedToday.reduce((s, e) => s + e.amount, 0);
      const balance = computeTrackBalance(sec, topic, task);
      learningRows.push({ topicTitle: topic.title, trackTitle: sec.title, unit: sec.unit, todaySum, quota: sec.quotaPerPeriod, balance });
    });
  });
  const lines = [];
  ...
  if (todaysPlan.length === 0) lines.push("_\u0647\u06CC\u0686 \u062A\u0633\u06A9\u06CC \u0628\u0631\u0627\u06CC \u0627\u0645\u0631\u0648\u0632 \u0628\u0631\u0646\u0627\u0645\u0647\u200C\u0631\u06CC\u0632\u06CC \u0646\u0634\u062F\u0647._");
  ...
  if (learningRows.length === 0) {
    lines.push("_\u0627\u0645\u0631\u0648\u0632 \u067E\u06CC\u0634\u0631\u0641\u062A\u06CC \u0628\u0631\u0627\u06CC \u0645\u0633\u06CC\u0631\u0647\u0627\u06CC \u06CC\u0627\u062F\u06AF\u06CC\u0631\u06CC \u062B\u0628\u062A \u0646\u0634\u062F\u0647._");
  } else {
    learningRows.forEach((r) => {
      let balanceText = "";
      if (r.balance) {
        if (r.balance.balance > 0) balanceText = ` \u2014 \u0639\u0642\u0628: ${r.balance.balance} ${r.unit}`;
        else if (r.balance.balance < 0) balanceText = ` \u2014 \u062C\u0644\u0648: ${-r.balance.balance} ${r.unit}`;
        else balanceText = " \u2014 \u0645\u0637\u0627\u0628\u0642 \u0628\u0631\u0646\u0627\u0645\u0647";
      }
      const quotaText = r.quota ? ` (\u0633\u0647\u0645\u06CC\u0647: ${r.quota} ${r.unit})` : "";
      lines.push(`- ${r.topicTitle} \u2014 ${r.trackTitle}: ${r.todaySum} ${r.unit}${quotaText}${balanceText}`);
    });
  }
  lines.push("", `_\u0635\u0627\u062F\u0631\u0634\u062F\u0647 \u062A\u0648\u0633\u0637 \u067E\u0644\u0627\u06AF\u06CC\u0646 \u0632\u0646\u062F\u06AF\u06CC\u0622\u0631\u0627\u0645 \u062F\u0631 ${(/* @__PURE__ */ new Date()).toLocaleTimeString("fa-IR")}_`);
  return lines.join("\n");
...
function buildMonthlyMarkdownReport({ tasks, projects, pomodoro }) {
  const end = /* @__PURE__ */ new Date();
  end.setHours(0, 0, 0, 0);
  const days = [];
  for (let i = 29; i >= 0; i--) {
  ...
  (projects || []).forEach((topic) => {
    (topic.subsections || []).forEach((sec) => {
      ...
      const monthQuota = dueDays * sec.quotaPerPeriod;
      const monthSum = (task.progressLog || []).filter((e) => e.date >= startKey && e.date <= endKey).reduce((s, e) => s + e.amount, 0);
      if (monthQuota === 0 && monthSum === 0) return;
      learningRows.push({ topicTitle: topic.title, trackTitle: sec.title, unit: sec.unit, monthSum, monthQuota });
    });
  });
  const lines = [];
  ...
  if (learningRows.length === 0) {
    ...
    learningRows.forEach((r) => {
      const diff = r.monthQuota - r.monthSum;
      let diffText = "";
      if (diff > 0) diffText = ` \u2014 \u0639\u0642\u0628: ${diff} ${r.unit}`;
      else if (diff < 0) diffText = ` \u2014 \u062C\u0644\u0648: ${-diff} ${r.unit}`;
      else diffText = " \u2014 \u0645\u0637\u0627\u0628\u0642 \u0628\u0631\u0646\u0627\u0645\u0647";
      lines.push(`- ${r.topicTitle} \u2014 ${r.trackTitle}: ${r.monthSum} \u0627\u0632 ${r.monthQuota} ${r.unit}${diffText}`);
    });
  }
  lines.push("", `_\u0635\u0627\u062F\u0631\u0634\u062F\u0647 \u062A\u0648\u0633\u0637 \u067E\u0644\u0627\u06AF\u06CC\u0646 \u0632\u0646\u062F\u06AF\u06CC\u0622\u0631\u0627\u0645 \u062F\u0631 ${(/* @__PURE__ */ new Date()).toLocaleTimeString("fa-IR")}_`);
  return lines.join("\n");
...
function DailyReportView({ projects, tasks, pomodoro, onAddProgress }) {
  ...
  const exportReport = async () => {
    ...
    try {
      const plugin = typeof window !== "undefined" && window.__lifeflowPlugin;
      if (!plugin || typeof plugin.exportDailyReport !== "function") {
        setExportErr("\u0627\u06CC\u0646 \u0642\u0627\u0628\u0644\u06CC\u062A \u0641\u0642\u0637 \u062F\u0627\u062E\u0644 \u062E\u0648\u062F \u067E\u0644\u0627\u06AF\u06CC\u0646 Obsidian \u06A9\u0627\u0631 \u0645\u06CC\u200C\u06A9\u0646\u062F.");
        return;
      }
      const content = buildDailyMarkdownReport({ tasks, projects, pomodoro });
      const path = await plugin.exportDailyReport(`${todayKey()}.md`, content);
      setExportMsg(`\u0630\u062E\u06CC\u0631\u0647 \u0634\u062F \u062F\u0631: ${path}`);
      setTimeout(() => setExportMsg(""), 4e3);
    } catch (e) {
  ...
  const exportMonthlyReport = async () => {
    ...
    try {
      const plugin = typeof window !== "undefined" && window.__lifeflowPlugin;
      if (!plugin || typeof plugin.exportDailyReport !== "function") {
        setMonthlyErr("\u0627\u06CC\u0646 \u0642\u0627\u0628\u0644\u06CC\u062A \u0641\u0642\u0637 \u062F\u0627\u062E\u0644 \u062E\u0648\u062F \u067E\u0644\u0627\u06AF\u06CC\u0646 Obsidian \u06A9\u0627\u0631 \u0645\u06CC\u200C\u06A9\u0646\u062F.");
        return;
      }
      const content = buildMonthlyMarkdownReport({ tasks, projects, pomodoro });
      const path = await plugin.exportDailyReport(`monthly-${todayKey()}.md`, content);
      setMonthlyMsg(`\u0630\u062E\u06CC\u0631\u0647 \u0634\u062F \u062F\u0631: ${path}`);
      setTimeout(() => setMonthlyMsg(""), 4e3);
    } catch (e) {
...
function NoteListsBoard({ noteLists, setNoteLists }) {
  ...
  return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("button", { onClick: () => setShowNew(true), className: "mod-cta w-full flex items-center justify-center gap-2 rounded-xl py-3 mb-4 text-sm font-bold text-white" }, /* @__PURE__ */ React.createElement(Ic, { name: "plus", size: 16 }), " \u0644\u06CC\u0633\u062A \u062C\u062F\u06CC\u062F"), noteLists.length === 0 && /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-500 text-center py-8" }, "\u0647\u0646\u0648\u0632 \u0644\u06CC\u0633\u062A\u06CC \u0646\u0633\u0627\u062E\u062A\u06CC \u2014 \u0645\u062B\u0644\u0627\u064B \xAB\u06A9\u062A\u0627\u0628\u200C\u0647\u0627\u06CC \u0645\u06CC\u200C\u062E\u0648\u0627\u0645 \u0628\u062E\u0648\u0646\u0645\xBB \u06CC\u0627 \xAB\u062E\u0631\u06CC\u062F\xBB \u0628\u0633\u0627\u0632"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3" }, noteLists.map((list) => /* @__PURE__ */ React.createElement(NoteListCard, { key: list.id, list, onUpdate: updateList, onDelete: deleteList }))), showNew && /* @__PURE__ */ React.createElement(NewListModal, { onClose: () => setShowNew(false), onCreate: createList }));
}
...
// Splits 100% evenly across `n` tracks so the shares always sum to exactly
// 100 (any leftover 1%-units go to the first slots, e.g. 3 -> [34,33,33])
// rather than drifting from rounding. This share is purely for later
// reporting ("how much of my pomodoro time went to each track") \u2014 the
// actual progress amount logged per track is entered independently by
// the user in that track's own unit, not derived by splitting one number
// (tracks can have completely different units: pages vs. words vs. reps).
...
function PomodoroTimerView({ pomodoro, setPomodoro, tasks, onAddProgress, onToggle, notifSettings, onFocusChange }) {
  ...
  const multiShares = computeEqualShares(multiTaskIds.length);
  ...
  const logSession = (completed) => {
    const useMulti = mode === "work" && multiMode && multiTaskIds.length > 0;
    const entry = {
      id: uid(),
      type: mode,
      taskId: mode === "work" && !useMulti ? taskId || null : null,
      taskAllocations: useMulti ? multiTaskIds.map((id, i) => ({ taskId: id, sharePct: multiShares[i] })) : null,
  ...
  const multiToggle = React.createElement(
    "button",
    {
      type: "button",
      onClick: () => {
      ...
      style: { borderColor: multiMode ? "var(--interactive-accent)" : "var(--background-modifier-border)", color: multiMode ? "var(--text-accent)" : "var(--text-muted)" }
    },
    multiMode ? "\u062D\u0627\u0644\u062A \u0686\u0646\u062F\u0645\u0633\u06CC\u0631\u06CC \u0641\u0639\u0627\u0644" : "\u062A\u0642\u0633\u06CC\u0645 \u0628\u06CC\u0646 \u0686\u0646\u062F \u0645\u0633\u06CC\u0631"
  );
  const taskPickerHeader = React.createElement(
    "div",
    { className: "flex items-center justify-between mb-1.5" },
    React.createElement("p", { className: "text-slate-400 text-xs" }, t("pomodoro_assign_task", "fa")),
    multiToggle
  );
  const singlePicker = React.createElement(
    "select",
    {
      value: taskId,
    ...
    React.createElement("option", { value: "", className: "bg-[#120814]" }, t("pomodoro_no_task", "fa")),
    activeTasks.map((tk) => React.createElement("option", { key: tk.id, value: tk.id, className: "bg-[#120814]" }, tk.title, tk.progressType === "progressive" ? ` (${tk.progressCurrent || 0}/${tk.progressTarget} ${tk.progressUnit})` : ""))
  );
  const multiPicker = React.createElement(
    "div",
    { className: "space-y-1.5", style: { maxHeight: 160, overflowY: "auto" } },
    progressiveActiveTasks.length === 0 ? React.createElement("p", { className: "text-[11px] text-slate-600" }, "\u0647\u06CC\u0686 \u062A\u0633\u06A9 \u0631\u0648\u0646\u062F\u200C\u062F\u0627\u0631\u06CC \u0641\u0639\u0627\u0644\u06CC \u0648\u062C\u0648\u062F \u0646\u062F\u0627\u0631\u062F") : progressiveActiveTasks.map((tk) => {
      const idx = multiTaskIds.indexOf(tk.id);
      const checked = idx !== -1;
      return React.createElement(
        "label",
        { key: tk.id, className: "flex items-center gap-2 text-xs px-2 py-1.5 rounded-lg", style: { background: checked ? "var(--background-modifier-hover)" : "transparent", cursor: running ? "default" : "pointer" } },
        React.createElement("input", { type: "checkbox", checked, disabled: running, onChange: () => toggleMultiTask(tk.id) }),
        React.createElement("span", { className: "flex-1 truncate" }, tk.title),
        checked && React.createElement("span", { className: "text-[10px] font-bold", style: { color: "var(--text-accent)" } }, `${toFa(multiShares[idx])}\u066A`)
      );
    })
  );
...
function CalendarHeader({ view, cursor, onPrev, onNext, onToday, onView }) {
  ...
  return /* @__PURE__ */ React.createElement(GlassCard, { className: "p-3 flex items-center justify-between flex-wrap gap-2" }, navButtons || React.createElement("div", null), /* @__PURE__ */ React.createElement("p", { className: "text-sm font-bold text-slate-100" }, title), /* @__PURE__ */ React.createElement("div", { className: "flex bg-white/[0.05] border border-white/10 rounded-xl p-1" }, [["day", "\u0631\u0648\u0632"], ["threeDay", "\u06F3\u0631\u0648\u0632\u0647"], ["week", "\u0647\u0641\u062A\u0647"], ["month", "\u0645\u0627\u0647"], ["year", "\u0633\u0627\u0644"], ["agenda", "\u0641\u0647\u0631\u0633\u062A"], ["custom", "\u0633\u0641\u0627\u0631\u0634\u06CC"]].map(([v, l]) => /* @__PURE__ */ React.createElement("button", { key: v, onClick: () => onView(v), className: `px-2.5 py-1.5 rounded-lg text-[11px] font-medium ${view === v ? "bg-white/10 text-white" : "text-slate-400"}` }, l))));
}
...
// appearance setting.
// بندهای ۲۰/۲۳/۱۹ مشترکاً از این دو لیبل استفاده می‌کنند (CalendarZoomControls
// و CustomViewEditorModal تازه‌ی بندِ ۱۹).
function calSlotLabel(m) {
  return m === 60 ? "\u06F1\u0633\u0627\u0639\u062A" : `${toFa(m)}\u062F`;
}
function calDetailLabel(d) {
  return d === "full" ? "\u06A9\u0627\u0645\u0644" : d === "compact" ? "\u0641\u0634\u0631\u062F\u0647" : "\u06A9\u0645\u06CC\u0646\u0647";
}
function CalendarZoomControls({ zoom, slotMinutes, taskDetail, onZoomChange, onSlotMinutesChange, onTaskDetailChange }) {
  ...
  const slotLabel = calSlotLabel;
  const detailLabel = calDetailLabel;
...
// بند ۱۹ (نماهای کاملاً سفارشی): فرمِ ساخت/ویرایشِ یک نمای ذخیره‌شده.
...
function CustomViewEditorModal({ initial, tasks, onClose, onSave }) {
  ...
  const [typeFilter, setTypeFilter] = useState(initial.typeFilter || null);
  ...
  const toggleType = (t2) => setTypeFilter((prev) => {
    const cur = Array.isArray(prev) ? prev : [];
    return cur.includes(t2) ? cur.filter((x) => x !== t2) : [...cur, t2];
  });
  const submit = () => {
    onSave({
      ...
      typeFilter: Array.isArray(typeFilter) && typeFilter.length > 0 ? typeFilter : null
    });
  };
  return /* @__PURE__ */ React.createElement(
    ModalShell,
    ...
    /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs text-slate-400" }, "\u062A\u0639\u062F\u0627\u062F \u0631\u0648\u0632"), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 bg-white/[0.05] border border-white/10 rounded-xl p-1" }, /* @__PURE__ */ React.createElement(
      "button",
      { onClick: () => setDayCount((d) => Math.max(1, d - 1)), className: "w-7 h-7 rounded-lg flex items-center justify-center text-slate-300" },
      /* @__PURE__ */ React.createElement(Ic, { name: "minus", size: 13 })
    ), /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-slate-300 w-9 text-center" }, toFa(dayCount), " \u0631\u0648\u0632"), /* @__PURE__ */ React.createElement(
      "button",
      { onClick: () => setDayCount((d) => Math.min(14, d + 1)), className: "w-7 h-7 rounded-lg flex items-center justify-center text-slate-300" },
      /* @__PURE__ */ React.createElement(Ic, { name: "plus", size: 13 })
    ))),
    /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs text-slate-400" }, "\u0633\u0637\u062D \u062C\u0632\u0626\u06CC\u0627\u062A"), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 bg-white/[0.05] border border-white/10 rounded-xl p-1" }, CAL_TASK_DETAIL_OPTIONS.map((d) => /* @__PURE__ */ React.createElement(
      "button",
      { key: d, onClick: () => setTaskDetail(d), className: `px-2.5 py-1 rounded-lg text-[11px] font-medium ${taskDetail === d ? "bg-white/10 text-white" : "text-slate-400"}` },
      calDetailLabel(d)
    )))),
    /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between mb-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs text-slate-400" }, "\u0645\u0642\u06CC\u0627\u0633 \u0632\u0645\u0627\u0646\u06CC"), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1 bg-white/[0.05] border border-white/10 rounded-xl p-1" }, CAL_SLOT_OPTIONS.map((m) => /* @__PURE__ */ React.createElement(
      "button",
    ...
    /* @__PURE__ */ React.createElement("div", { className: "mb-1" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-slate-400 mb-1.5" }, "\u0641\u0642\u0637 \u0627\u06CC\u0646 \u0646\u0648\u0639 \u062A\u0633\u06A9 \u0646\u0634\u0627\u0646 \u062F\u0627\u062F\u0647 \u0634\u0648\u062F (\u062E\u0627\u0644\u06CC = \u0647\u0645\u0647)"), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-1.5" }, /* @__PURE__ */ React.createElement(Chip, { active: Array.isArray(typeFilter) && typeFilter.includes("binary"), onClick: () => toggleType("binary") }, "\u0633\u0627\u062F\u0647"), /* @__PURE__ */ React.createElement(Chip, { active: Array.isArray(typeFilter) && typeFilter.includes("progressive"), onClick: () => toggleType("progressive"), color: "#22D3EE" }, "\u0631\u0648\u0646\u062F\u062F\u0627\u0631"))),
...
function CalendarViews({ tasks, onToggle, onSchedule, onDelete, onEdit, onAddProgress, onCreateAt, appearance, onChangeAppearance, customViews = [], onAddCustomView, onUpdateCustomView, onDeleteCustomView }) {
  ...
  const [editingCustomView, setEditingCustomView] = useState(null);
  const activeCustomView = customViews.find((v) => v.id === activeCustomViewId) || null;
  ...
  const step = (dir) => {
    ...
    else if (view === "custom") setCursor((c) => Jalali.addDays(c, dir * (activeCustomView ? activeCustomView.dayCount : 1)));
  ...
  const customViewTasks = (cv) => {
    ...
    if (Array.isArray(cv.typeFilter) && cv.typeFilter.length > 0) result = result.filter((t2) => cv.typeFilter.includes(t2.progressType || "binary"));
    return result;
  };
  const customToolbar = React.createElement(
    "div",
    { className: "flex items-center gap-1.5 flex-wrap" },
    customViews.map((cv) => React.createElement(
      "button",
      {
        key: cv.id,
        onClick: () => setActiveCustomViewId(cv.id),
        className: `px-2.5 py-1.5 rounded-lg text-[11px] font-medium shrink-0 ${activeCustomViewId === cv.id ? "bg-white/10 text-white" : "bg-white/[0.05] text-slate-400"}`
      },
      cv.name || "\u0628\u06CC\u200C\u0646\u0627\u0645"
    )),
    React.createElement(
      "button",
      {
        onClick: () => setEditingCustomView({ id: uid(), name: "", dayCount: 3, taskDetail: "full", zoom: 1, slotMinutes: 30, quadFilter: null, tagFilter: null, typeFilter: null, isNew: true }),
    ...
    activeCustomView && React.createElement(
      ...
      {
        onClick: () => {
          onDeleteCustomView && onDeleteCustomView(activeCustomView.id);
          setActiveCustomViewId(null);
        },
        className: "w-7 h-7 rounded-lg bg-white/[0.05] flex items-center justify-center shrink-0",
        title: "\u062D\u0630\u0641"
      },
      React.createElement(Ic, { name: "trash", size: 13, className: "text-rose-400/80" })
    )
  );
  const customContent = customViews.length === 0 ? React.createElement(GlassCard, { className: "p-6 text-center" }, React.createElement("p", { className: "text-sm text-slate-400 mb-3" }, "\u0647\u0646\u0648\u0632 \u0647\u06CC\u0686 \u0646\u0645\u0627\u06CC \u0633\u0641\u0627\u0631\u0634\u06CC\u06CC \u0646\u0633\u0627\u062E\u062A\u0647\u200C\u0627\u06CC\u062F"), React.createElement(
    "button",
    { onClick: () => setEditingCustomView({ id: uid(), name: "", dayCount: 3, taskDetail: "full", zoom: 1, slotMinutes: 30, quadFilter: null, tagFilter: null, typeFilter: null, isNew: true }), className: "mod-cta rounded-xl px-4 py-2 text-sm font-bold" },
    ...
    cursor,
    tasks: customViewTasks(activeCustomView),
    onEdit,
    onCreateAt,
    onSchedule,
    dayCount: activeCustomView.dayCount,
    zoom: activeCustomView.zoom,
    slotMinutes: activeCustomView.slotMinutes,
    taskDetail: activeCustomView.taskDetail
  });
...
function LifeFlowApp() {
  ...
  const isNavVisible = (n) => n.id === "dashboard" || n.id === "tasks" || settings.features?.tabs?.[n.id] !== false;
  useEffect(() => {
    const onExternalSettingsChange = () => setSettings(loadSettings());
    window.addEventListener("lifeflow-settings-changed", onExternalSettingsChange);
    return () => window.removeEventListener("lifeflow-settings-changed", onExternalSettingsChange);
  }, []);
  const lang = settings.language;
  ...
  // بند ۱۹ (نماهای کاملاً سفارشیِ تقویم): هر نما یک ترکیبِ ذخیره‌شده از
  // {dayCount, taskDetail, zoom, slotMinutes, quadFilter} است - همان
  // زیرساختِ بندهای ۲۰/۲۲/۲۳ (که از قبل روی WeekHourlyView قابلِ تنظیم
  // بودند)، فقط این‌بار به‌عنوانِ چند پیش‌تنظیمِ قابلِ‌نام‌گذاری و ذخیره،
  // به‌جای یک تنظیمِ سراسریِ واحد.
  const [customCalendarViews, setCustomCalendarViews] = useState(savedData.customCalendarViews || []);
  const addCustomCalendarView = (view) => setCustomCalendarViews((p) => [...p, view]);
  const updateCustomCalendarView = (id, patch) => setCustomCalendarViews((p) => p.map((v) => v.id === id ? { ...v, ...patch } : v));
  const deleteCustomCalendarView = (id) => setCustomCalendarViews((p) => p.filter((v) => v.id !== id));
  ...
  useEffect(() => {
    const fullState = { tasks, books, videos, podcasts, exercises, projects, planning, goals, journal, noteLists, calendars, automationRules, customActionButtons, customCalendarViews, pomodoro };
    try {
  ...
  }, [tasks, books, videos, podcasts, exercises, projects, planning, goals, journal, noteLists, calendars, automationRules, customActionButtons, customCalendarViews, pomodoro]);
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("style", null, `
      ...
      `),
    /* @__PURE__ */ React.createElement("div", { className: "hidden lg:flex flex-col w-56 shrink-0 h-full sticky top-0 px-4 py-6 z-10 glass-strong lg:rounded-none lg:border-l lg:border-t-0 lg:border-b-0 lg:border-r-0" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 px-2 mb-8" }, /* @__PURE__ */ React.createElement("img", { src: "./logo.png", alt: "", className: "w-8 h-8 rounded-lg" }), /* @__PURE__ */ React.createElement("span", { className: "font-extrabold text-lg" }, lang === "fa" ? "\u0632\u0646\u062F\u06AF\u06CC\u200C\u0622\u0631\u0627\u0645" : "LifeFlow")), /* @__PURE__ */ React.createElement("nav", { className: "flex flex-col gap-1 relative" }, NAV.filter(isNavVisible).map((n) => {
